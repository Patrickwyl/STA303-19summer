...
# So it kind of looks like lower temperatures have more failures. 
# Plot the data? How do we plot binomial data...?
orings_tbl %>%
...
#cbind -> matrix -> must be a binomial model
...
# ~ 1 means fit the null model: with only intercept beta0
...
# !!!
# So increasing temperature by 1F is associated with a new odds of 0.81 x the old odds
# A much better way to say this is increasing temp by 1F is
# associated with a (1 - 0.81 = 0.19) 19% reduction in
...
summary(wbca_glm1) #no cbind -> binary
...
Dstat #null -> only beta0
# Compare to a chisq(680 - 671) distribution
1 - pchisq(Dstat,9) #right hand side area in chisq
...
deviance(wbca_glm1)
1 - pchisq(deviance(wbca_glm1),671) # 680-9 chisq model
...
# Note though that the chisquare approximation is terrible for binary data, 
# and there are other problems in using deviance with binary data
...
# only beta0 vs saturated model 680-0
...
# stepwise method (forward and backword) automatically -> Fisher Scoring
...
# How to make a hard 0/1 prediction for each case? 
# !!!The textbook suggests using a cutoff of 0.5.
