# Supplementary R code for lecture 5, STA303 summer 2018
# Alex Stringer
# 
...
# !!!
# n=30, count not large: no fixed known max -> Poisson model
gala_tbl %>%
...
# But the diagnostics...
# these data don't have constant variance 
# two ways to solve-> 
# 1. try transformation 2.differnt model
...
# Transformation? 
# We have the variance stabilizing transformation. 
# Let's try the boxcox one too and compare them

gala_boxcox <- boxcox(gala_lm1)

# 0.5 is within the range of "good" values. 
# So, let's try a square root transformation!!!
...
# Much better.
# 
# Before we do any modelling, 
# really, we should at least take a look at the relationship
...
# n=30, p=6, n-p=24
# residual Deviance: D= 716.85 -> does the model fit well?
# Since 24 + 2sqrt(2(n-p)) = 38
# D >> 38 -> not fit as well as the saturated model.
...
# It looks like we have overdispersion -> variance too big
# 
# Estimate dispersion
n <- nrow(gala_tbl)
...
# Yes, lots of overdispersion (>> 1)
# Compute the new standard errors

summary(gala_glm1,dispersion = phi)
summary(gala_glm1)

# standard errors become much smaller
...
# Coefficient for log(Area) is about 3.994e-01 = 0.4
# This is probably too small to consider to be "close to 1"
# But the coefficient for offset should be 1.
...
# Dose increases exponentially; let's log it
salmonella_tbl %>%
